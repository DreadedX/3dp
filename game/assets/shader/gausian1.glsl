...
void main() {
	...
	for (int i = -12; i <= 12; i++) {
		vs_out.blurTextureCoords[i+12] = centerTexCoords + vec2(pixelSize * i, 0);

		vs_out.blurTextureCoords[i+12] = vec2( max(0.0, vs_out.blurTextureCoords[i+12].x), max(0.0, vs_out.blurTextureCoords[i+12].y) );
		vs_out.blurTextureCoords[i+12] = vec2( min(1.0, vs_out.blurTextureCoords[i+12].x), min(1.0, vs_out.blurTextureCoords[i+12].y) );
	}
}
